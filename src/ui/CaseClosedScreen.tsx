...
export function CaseClosedScreen({ script, epilogue, proved, total, onReplay }: Props) {
    ...
    >
      <View style={styles.record}>
        ...
        <View style={styles.tally}>
          {/* The same mark the tile and the accusation sheet draw. It has been the
              unit of evidence all game; on this screen it is the score. */}
          <View style={styles.marks}>
            {Array.from({ length: total }).map((_, i) => (
              <View key={i} style={[styles.mark, i >= proved && styles.markMissed]} />
            ))}
          </View>
          <Text style={styles.tallyText}>{t('closed.proved', { proved, total })}</Text>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  tally: { flexDirection: 'row', alignItems: 'center', gap: theme.space.sm },
  marks: { flexDirection: 'row', gap: 3 },
  mark: { width: 3, height: 16, borderRadius: 1.5, backgroundColor: theme.color.danger },
  ...
  tallyText: { ...theme.type.meta, color: theme.color.textDim },
  ...
  quietText: { ...theme.type.meta, color: theme.color.textDim },
